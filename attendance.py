...
class Attendance:
    def __init__(self, root,main_root):
        ...
        #export button
        ExportCSV_button=CTkButton(button_frame1,text='Delete',command=self.delete_fun,width=284,height=31,font=('times new roman',12),fg_color='blue',corner_radius=8,bg_color='white',text_color='white',hover_color='#0D52BD')
        ...
        showall_button=CTkButton(search_frame,text='Show All',command=self.show_all_data,width=110,font=('times new roman',12,'bold'),text_color='white',fg_color='blue',hover_color='#0D52BD')
    ...
    def importCSV(self):
        try:
            global mydata
            mydata.clear()
            self.file_name=filedialog.askopenfilename(initialdir=os.getcwd(),title='Open CSV',filetypes=(('CSV File','*.csv'),('All Files','*.*')),parent=self.root)
            if self.file_name:
    ...
    def exportCSV1(self):#O
        try:
            ...
            messagebox.showerror("Error",f"Error due to: {str(es)}",parent=self.root)
    def and_save(self, data_list):
        try:
            file_name = self.file_name  # Assuming self.file_name is the original file name
    ...
    def update_fun(self):
        if (self.dep_var.get() == 'Select Department' or self.name_var.get() == '' or 
            ...
            try:
                ...
                if ask_update:
                    updated = False
                    data_list = mydata[:]  # Create a copy of the imported data

                    for row in data_list:
                        if row[1] == self.std_id_var.get():  # Assuming ID is the second column
                            row[0] = self.name_var.get()
                            row[2] = self.A_status_var.get()
                            row[3] = self.dep_var.get()
                            row[4] = self.date_var.get()
                            row[5] = self.time_var.get()
                            updated = True
                            break

                    if updated:
                        self.and_save(mydata)  # Export the updated data to the original file
                        
                        self.fetchData(mydata)
                        self.reset_fun()  # Reset the form after update
                    else:
    ...
    def search_data(self):
        ...
        if self.search_by_var.get() == 'Select' or self.search_value_var.get()== '':
        ...
        try:
            ...
            for row in data_list:
                if (self.search_by_var.get() == 'Student ID' and row[1] == self.search_value_var.get()) or (self.search_by_var.get() == 'Status' and row[2] == self.search_value_var.get().title()):
    ...
    def delete_fun(self):
        if self.std_id_var.get()=="":
            ...
            if ask_delete:
                selected_item = self.attendance_table.selection()[0]
                self.attendance_table.delete(selected_item)
                for index, row in enumerate(mydata):
                    if list(row) == self.attendance_table.item(selected_item, 'values'):
                        del mydata[index]
                        break
